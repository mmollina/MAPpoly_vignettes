...
def real_main(argv):
    try:
        opts, args = getopt.getopt(argv, '', ['inference=', 'optimal_inference', 'file=', 'ploidy_range=', 'sigma_range=', 'draw_genotypes', 'print_genotypes', 'print_genotypes_histogram', 'draw_genotype_histogram', 'heights_or_areas=', 'naive_posterior_reporting_threshold=', 'display_progress', 'save_figures=', 'f1_parent_data=', 'slow_inference'])
    ...
    if inference == 'f1':
        ...
        if 'naive_posterior_reporting_threshold' in options_map:
            ...
            inds_to_genos = infer.naive_high_quality_genotypes(naive_reporting_thresh, inds_to_genos, parameters)
...
def cgi_main(argv):
    try:
        if '--draw_genotypes' in set(argv) or '--draw_genotype_histogram' in set(argv):
            temp_prefix='/home/orserang/www/imgs'
