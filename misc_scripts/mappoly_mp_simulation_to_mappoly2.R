mappoly_mp_simulation_to_mappoly2 <- function(x, file_path = NULL){
  ...
  w <- vector("list", length(bipar.pops))
  for(k in 1:length(x)){
    ...
    for(i in 1:length(bipar.pops)){
      ...
      pl.ind <- x[[k]]$pedigree[x[[k]]$pedigree[,1] == P1 & x[[k]]$pedigree[,2] == P2,3:4]
      pl <- unique(pl.ind)
      ...
      w[[i]]<-rbind(w[[i]], data.frame(snp_id = id.mrk.names,
  ...
  for(i in 1:length(bipar.pops)){
    ...
    write.csv(w[[i]], file = file_path_out, row.names = FALSE)
  }
}
